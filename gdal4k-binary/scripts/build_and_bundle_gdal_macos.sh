...
Options:
  ...
  --output-dir <path>      Bundle output directory (default: <module>/build/gdal-bundle)
    --gdal-version <ver>     GDAL version (default: 3.9.0)
...
setup_build_env() {
  ...
  if [[ "$is_windows_shell" == "1" ]]; then
    ...
    # MinGW does not understand the MSVC-style /utf-8 switch that can leak in
    # from conda's Windows activation scripts. Strip it while preserving any
    # other caller-provided compiler flags.
    for flag_var in CFLAGS CXXFLAGS CPPFLAGS; do
      local flag_value="${!flag_var:-}"
      if [[ -n "$flag_value" ]]; then
        flag_value="${flag_value//\/utf-8/}"
        flag_value="${flag_value//-utf-8/}"
        while [[ "$flag_value" == *"  "* ]]; do
...
download_gdal() {
  ...
  # The release tarball ships generated man pages, which CMake tries to install
  # into the active conda prefix. On GitHub-hosted Linux runners that prefix is
  # not writable for chmod/set-permission operations, so skip man page installs.
  if [[ -d "$gdal_src/man/man1" ]]; then
    rm -rf "$gdal_src/man/man1"
  fi
}
...
text = path.read_text()
replacements = [
    ("#include <xstring>", "#include <string>"),
    ("L''", "L'\\0'"),
    ("GetFileAttributes(", "GetFileAttributesW("),
    ("DeleteFile(", "DeleteFileW("),
    ("TCHAR lpPathBuffer[BUFSIZE];", "wchar_t lpPathBuffer[BUFSIZE];"),
    ("TCHAR szTempName[BUFSIZE];", "wchar_t szTempName[BUFSIZE];"),
    ("GetTempPath(", "GetTempPathW("),
    ("GetTempFileName(", "GetTempFileNameW("),
    ('TEXT("libkml")', 'L"libkml"'),
]
updated = text
...
install_conda_deps() {
  if [[ ! -f "$conda_prefix/conda-meta/history" ]]; then
    case "$conda_prefix" in
      "$project_dir"/build/*)
        rm -rf "$conda_prefix"
        retry 3 env CONDA_NO_PLUGINS=true "$conda_exe_path" create -y -p "$conda_prefix" --solver=classic python=3.12
        ;;
      *)
        echo "Conda prefix exists but is not a conda environment: $conda_prefix" >&2
        exit 1
        ;;
    esac
  fi
